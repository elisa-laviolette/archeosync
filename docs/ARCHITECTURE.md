...
- **Alternative objects robustness**: Case-insensitive field mapping is applied when converting alternative-object rows to Objects schema. At import, object duplicates are detected by comparing **all field values** (excluding geometry, `fid`/`ogc_fid`, and virtual/computed fields). Within the import batch, a no-geometry alternative-object row is excluded when a geometric object with the same attribute signature is already kept; geometric rows are preferred over no-geometry rows. The same attribute comparison applies to no-geometry vs no-geometry duplicates in the batch and against definitive no-geometry rows in the Objects layer. A no-geometry import is also excluded when a definitive polygon already carries the same attributes; it is **kept** when only zone/number overlap but other attributes differ (the duplicate-objects warning still applies for partial matches). Rows with no comparable attributes remain ambiguous and are kept. Alternative-object GeoPackages are merged whenever present (not only when global metadata exists).
- **GeoPackage loading**: Import tries configured layer names and file basenames first, then other OGR sublayers only as a fallback (so a re-import after edits does not read a different table that still has more rows). Before reading, each source `.gpkg` is snapshotted with SQLite's online backup API (including uncheckpointed WAL changes) so QGIS's global OGR connection pool cannot return stale data after a cancelled import.
...
- **Temporary Layer Configuration**: For each temporary import layer (`New Objects`, etc.), snapshots definitive project relations, clones them with new ``archeosync_import_*`` ids (remapping definitive layer ids to temporary ids and, when present, peer temporary import layers via ``build_peer_temp_layer_replacements``), restores any definitive relation altered by QGIS, then applies **safe** styling via ``configure_temporary_field_import_layer`` (renderer, display expression, overlapping field widgets, and relation remapping only — **no full QML/form load**, which can crash QGIS when the merged GeoPackage schema differs from the main-project definitive layers). ``configure_temporary_import_layer`` (full QML copy) remains available for other workflows but is not used during field-project import. Virtual fields for warning detectors are synced later from the import summary refresh pipeline via ``copy_virtual_fields``. After validation, ``repair_definitive_project_relations`` reconciles any definitive relation QGIS repointed or removed during import, then import-clone relations are removed before temporary layers are deleted so definitive project relations remain intact.
...
- **Multi-Layer Detection**: Detects duplicates within the "New Objects" layer and conflicts between that layer and the definitive "Objects" layer (the definitive layer is not scanned for internal duplicates)
...
- **Duplicate topo filtering**: Before creating ``Imported_CSV_Points``, CSV rows that match an existing definitive topo point on identifier (``PtID``, ``identifier``, etc.), geometry, and survey day are excluded. Duplicates within the same import batch are filtered the same way. The import summary reports the number of ignored topo duplicates via ``csv_duplicates``.
...
- **Menu entry behavior**: The Import Data menu opens this dialog only when no pending temporary import layers (`New Objects`, `New Features`, `New Small Finds`, `Imported_CSV_Points`) exist in the project; otherwise the plugin asks whether to continue the pending review or discard it and start a new import. When the user accepts this dialog for a **new** import, any leftover temporary layers are removed first so source GeoPackages are re-read from disk instead of reusing stale memory layers from a cancelled session. Selections are cached before the dialog closes; import processing is scheduled on the next Qt event-loop tick so layer/relation work does not run in the same turn as ``accept()`` (avoids hard crashes on macOS QGIS 4).
...
- **Skipped object numbers** (`SkippedNumbersDetectorService`): When a temporary "New Objects" layer exists, numbering gaps are evaluated on the union of definitive and temporary numbers per recording area; only gaps that are **not** already present in the definitive objects layer alone are reported, so pre-existing holes in project data do not produce import warnings. A gap immediately after a consecutive definitive block and before the next imported number is still reported even when a higher definitive number already implied a wider hole (e.g. definitive `1-3` and `10`, imported `5` warns for `4` but not for `6-9`).
- **Validation and Layer Copying**: Feature validation and copying workflow
  ...
  - **Incremental validation**: The Validate action loads temporary-layer features in small chunks, then copies **one feature per** ``QTimer`` tick on the Qt main thread so the progress bar can repaint between steps. The bar advances during both ``Loading {layer}…`` and ``Validating import…``. Target-layer signals are blocked and map canvas redraws are suspended during copying.
  - **Batched processing**: Each batch still builds features with ``QgsVectorLayerUtils.createFeature`` (per-feature default expressions) and inserts with ``addFeature`` one at a time so sequential aggregate defaults (e.g. ``maximum("sequence") + 1``) observe entities already in the edit buffer.
  ...
  - **Feature Selection**: Automatically selects newly copied features with ``selectByIds`` (including temporary negative FIDs from the edit buffer) for easy identification
  - **User Control**: Allows users to save or cancel changes as needed
  - **Cancel Import**: Removes all temporary import layers, clears pending archive paths for the cancelled session, and drops layer-service metadata caches so a subsequent import reads updated field-project GeoPackages instead of reusing stale data
  - **Dock lifecycle**: Closing the summary panel with the window **X** (without **Cancel Import** or **Validate**) keeps temporary import layers in the project but calls ``abort_async_operations()`` so in-flight warning refresh (``QgsTask``) and incremental validation (``QTimer``) cannot touch freed UI. **Import Data** always disposes hidden or orphaned summary docks before opening the next step. Reopening with pending layers rebuilds the dock without auto-running warning detectors (use **Refresh warnings** when ready). Replacing an existing dock aborts its background work before ``deleteLater()`` — ``processEvents()`` is not forced immediately afterward. When the user opens another QGIS project, ``ArcheoSyncPlugin`` listens to ``QgsProject.aboutToBeCleared``, ``iface.projectRead``, and ``iface.newProjectCreated`` (not ``iface.newProject()``, which is an action method in QGIS 4) to dispose summary docks and call ``reset_import_session_tracking()``.
...
- **Comprehensive Statistics Display**: Shows detailed import statistics after successful data import operations
  ...
  - **Out-of-bounds warnings**: When `OutOfBoundsDetectorService` reports features, objects, small finds, or total-station (topo) points outside their recording area (within the configured tolerance), messages and Select and show entities actions appear at the **top** of the summary scroll area, and after Refresh warnings the same block is rebuilt from `ImportSummaryData.out_of_bounds_warnings`. Reopening Import Data while temporary layers are pending **rebuilds** the dock so warnings stay in sync. Only pending temporary import layers (`New Objects`, `New Features`, `New Small Finds`, `Imported_CSV_Points`) are scanned; definitive project layers are never walked for spatial checks. Layers with no geometry skip recording-area indexing entirely. Topo points use `Imported_CSV_Points` when present; when no direct QGIS relation links the configured total station layer to recording areas, the shortest multi-hop relation chain is followed (same approach as distance warnings). Definitive layers on that chain are used only to resolve relation links, not to test feature positions.
  ...
  - **Distance warnings (import scope)**: During a pending import, distance checks use `Imported_CSV_Points` when present (paired with `New Objects` and/or the definitive objects layer). An **object-only** import (`csv_points_count` = 0) ignores any leftover `Imported_CSV_Points` layer from a previous session and runs distance checks only when the objects layer exposes topo link columns (`first_identifier` / `last_identifier` or common French variants); otherwise no distance warnings are produced. The import summary passes `csv_points_count` / `objects_count` into `DistanceDetectorService` for this scoping. Relation fields that reference recording areas are never used for point–object pairing.
  - **Refresh warnings**: Before re-running detectors, the dock calls the same virtual-field sync from definitive layers onto pending import layers as `ArcheoSyncPlugin._show_import_summary`, so refresh results match the initial summary run. Each refresh **replaces** the stored warning lists with the latest detector output when the pipeline finishes (for example, duplicate-object warnings disappear after the conflicting features are removed from `New Objects`). While detectors still run, the panel shows import-time warnings for categories that returned no new results yet, and shows fresh detector output immediately when a step finds issues, so unrelated warning groups do not vanish as the progress bar advances.
