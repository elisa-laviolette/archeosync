...
- **Comprehensive Statistics Display**: Shows detailed import statistics after successful data import operations
  ...
  - **Refresh warnings**: Before re-running detectors, the dock calls the same virtual-field sync from definitive layers onto pending import layers as `ArcheoSyncPlugin._show_import_summary`, so refresh results match the initial summary run. Each refresh **replaces** the stored warning lists with the latest detector output (for example, duplicate-object warnings disappear after the conflicting features are removed from `New Objects`). Completed detector steps update the panel incrementally without clearing warnings from other steps that are still pending, so duplicate-object and skipped-number warnings remain visible together when both apply.
